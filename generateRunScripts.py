    ...
    for num_procs in total_proc_choices:
        ...
        for i in range(1,4):
            ...
            if (app_1_singleProcRuntime < RUNTIME_OVERLAP_THRESHOLD*app_2_singleProcRuntime):
                ...
                slurm_file.write(f"srun --mpibind=off --ntasks {num_procs} --ntasks-per-node {ntasks_per_node} --nodes {num_nodes} --cpus-per-task 1 --mem 120G {app_2_exec} {app_2_inputs} | grep measuredTime >> {dir_path}/{app_2}_time.log &\n")
                ...
                slurm_file.write(f"\tsrun --mpibind=off --ntasks {num_procs} --ntasks-per-node {ntasks_per_node} --nodes {num_nodes} --cpus-per-task 1 --mem 120G {app_1_exec} {app_1_inputs} | grep measuredTime >> {dir_path}/{app_1}_time.log\n")
...
for app in apps:
    ...
    for num_procs in total_proc_choices:
        ...
        for i in range(1,4):
            ...
            slurm_file.write(f"srun --mpibind=off --ntasks {num_procs} --ntasks-per-node {ntasks_per_node} --nodes {num_nodes} --cpus-per-task 1 --mem 120G {app_exec} {app_input} | grep measuredTime >> {dir_path}/{app}_time.log &\n")
            slurm_file.write("wait\n")
            slurm_file.close()
            chdir("../")
