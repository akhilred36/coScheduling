        ...
        for msg_size in inhib_msg_sizes:
            for wait_time in inhib_wait_times:
                ...
                for i in range(1, 4):
                    ...
                    slurm_file.write(f"srun --mpibind=off --ntasks {num_procs} --ntasks-per-node {int(ntasks_per_node/2)} --nodes {num_nodes} --cpus-per-task 1 --mem 120G {inhib_exec} -m {msg_size} -w {wait_time} &\n")
                    ...
                    slurm_file.write(f"srun --mpibind=off --ntasks {num_procs} --ntasks-per-node {int(ntasks_per_node/2)} --nodes {num_nodes} --cpus-per-task 1 --mem 120G {app_1_exec} {app_1_inputs} | grep measuredTime >> {dir_path}/{app_1}_time.log\n")
