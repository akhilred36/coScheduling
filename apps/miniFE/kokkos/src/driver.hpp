...
//   In other words, a 2x2x2 box describes 8 elements, each of which has 8
//   nodes, so it is a 3x3x3 node domain (27 nodes). The assembled linear system
//   will have 1 equation for each finite element node.
//
           ...
           YAML_Doc& ydoc)
{
  ...
  RUN_TIMED_FUNCTION("imposing Dirichlet BC...",
                     impose_dirichlet(0.0, A, b, global_nx + 1, global_ny + 1,
                                      global_nz + 1, mesh.v_bc_rows_0),
                     dirbc_time, t_total);
  RUN_TIMED_FUNCTION("imposing Dirichlet BC...",
                     impose_dirichlet(1.0, A, b, global_nx + 1, global_ny + 1,
                                      global_nz + 1, mesh.v_bc_rows_1),
                     dirbc_time, t_total);
#ifdef MINIFE_DEBUG
  ...
  LocalOrdinal max_iters = params.max_iters;
  ...
  {
    std::cout << "ERROR, matvec with overlapping comm/comp is not implemented "
                 "in MiniFE/Kokkos."
              << std::endl;
  }
  else
  {
    cg_solve(A, b, x, max_iters, tol, num_iters, rnorm, cg_times);
    if (myproc == 0)
    {
  ...
  if (myproc == 0)
  {
    ...
    double dot_mflops = -1;
    if (cg_times[DOT] > 1.e-4) dot_mflops = 1.e-6 * (dot_flops / cg_times[DOT]);
