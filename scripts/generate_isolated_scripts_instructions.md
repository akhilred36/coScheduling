...
Write a python script, scripts/generate_isolated_scripts.py to do as follows. Make sure to ask any questions if instructions are unclear.
...
3) "walltime": set to "00:30:00"
4) "email": set to "aalasand1@unm.edu"
5) "num_cpus": set to 56
6) "mem": "120G"
...
4) Iterate through "node_choices", and the iterating variable is called "num_nodes":
    ...
    2) Iterate through all of the applications, the iterating variable is "a":
        1) Iterate from i=0 to i < redundant_runs:
            1) Create a file in "slurm_scripts" titled "num_nodes_a_i.slurm" where num_nodes, a, and i are the variables from the iterations.
            2) Create a directory in "data" titled "num_nodes_a_i" using the same variables. Create a directory inside this newly created directory titled "mpip_profiles"
            3) Now, write the following header to the created slurm file:
            ```
            #!/bin/bash
            #SBATCH --job-name <num_nodes>_<a>_<i>_isolated
            #SBATCH --mail-user <email>
            #SBATCH --mail-type FAIL,TIME_LIMIT
            #SBATCH --output <num_nodes>_<a>_<i>_isolated.out
            #SBATCH --error <num_nodes>_<a>_<i>_isolated.err
