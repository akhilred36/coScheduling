...
"""
            ...
            srun_command = f"time srun -n {num_cpus*num_nodes} --ntasks-per-node {num_cpus} --nodes {num_nodes} --mem {int(mem/2)-1}G --distribution=block:block --mpibind=on,v env LD_PRELOAD=\"{mpip_path}\" MPIP=\"-f {mpip_profiles_dir}\" {exec_full_path}{args_str} > {output_log} 2>&1"
