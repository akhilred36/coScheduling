...
def generate_slurm_content(num_nodes, app_a, app_b, run_id, apps_dict, data_dir):
    ...
    # Run app a
    srun_a = f'time srun -n {tasks_per_app} --mem {mem_per_app}G --nodes {num_nodes} --ntasks-per-node {num_cpus // 2} --distribution=block:block --mpibind=on,v env LD_PRELOAD="{mpip_path}" MPIP="-f {mpip_profile_dir}" {exec_a} {args_a} > {output_log_a} 2>&1 &'
    script_lines.append(srun_a)

    # Run app b
    srun_b = f'time srun -n {tasks_per_app} --mem {mem_per_app}G --nodes {num_nodes} --ntasks-per-node {num_cpus // 2} --distribution=block:block --mpibind=on,v env LD_PRELOAD="{mpip_path}" MPIP="-f {mpip_profile_dir}" {exec_b} {args_b} > {output_log_b} 2>&1 &'
