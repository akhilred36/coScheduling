...
srun --exclusive -n 56 --mem 119G --nodes {NUM_NODES} --ntasks-per-node {56} --distribution=block:block --mpibind=on,v  env LD_PRELOAD="{NETWORK_INHIBITOR_EXEC}" MPIP="{MPIP_FLAGS} {mpip_prof_path}" {inhib_exec} {inhib_args_str} > {data_dir}/inhib_output.log 2>&1 &

# Run the application with MPIP profiling in foreground
time srun --exclusive -n 56 --mem 119G --nodes {NUM_NODES} --ntasks-per-node {56} --distribution=block:block --mpibind=on,v env LD_PRELOAD="{NETWORK_INHIBITOR_EXEC}" MPIP="{MPIP_FLAGS} {mpip_prof_path}" {app_exec} {app_args_str} > {data_dir}/app_output.log 2>&1
